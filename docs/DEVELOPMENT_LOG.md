...
### M0 勘误: NP 固定样本口径坐标错位 + dyd[T−1] 堆越界读 (069264c, 2026-08-18)

> M0 收尾全量 ctest 拦截: 主控站 2255/2256, `NgPerronTest.DefaultParameters` 概率性失败 (单独重跑通过, 8 次循环 3 败) — 三平台收尾门禁流程的价值实证

- **根因 (逐层隔离诊断)**: MAIC 循环 `lhs[i] = dyd[t1−1]`, t1 (1-based) 最大取 T ⇒ 读 `dyd[T−1]`, 但 dyd 合法下标 0..T−2 — **堆越界读**, 垃圾值进入回归 lhs 末观测 ⇒ σ̂²(k)/MAIC/四统计量整体漂移; 进程内堆布局固定故结果稳定 (DefaultParameters 两次调用可同可异), 跨进程 ASLR ⇒ 概率性 (主控站 3/8, B 站 GCC 359s 全量恰未触发); 诊断链: 独立 diag exe 复现 (5/15) → gls_detrend/数据生成逐一排除 (A/B 项 0 差异) → σ̂²(0) 漂移锁定回归输入层 → 索引逐项复核实锤越界
- **连锁坐标错位**: lhs 与 X 滞后列的 Δỹ 索引整体偏 1 (1-based/0-based 混用); 原冻结口径 "t=k_max+1..T (1-based)" 下最深滞后需 Δỹ₁ (依赖不存在的 y₀) — spec 口径本身不可实现, 属 spec 勘误 (同 ZA Model A 勘误先例: 实施期发现, 回溯修正并记录依据)
- **修正口径**: 回归观测统一 0-based Δỹ 下标 idx ∈ [k_max, T−2], n = T−1−k_max (statsmodels adfuller 族一致); Σỹ² 同步回归 ỹ 列范围 [k_max, T−2]; ỹ_T 保持全样本最后观测 (头注明示)
- **StationaryArRejected 连锁修正**: T=300→500 — 修正后真实功率: T=300 MZα=−16.4 vs cv −17.3 **不拒绝** (原"通过"系越界垃圾虚增统计量, 是假阳性通过!), T=500 MZα=−33.5 稳定拒绝; 功率曲线一并落档 (T=500/1000/2000 → −33.5/−65.7/−115.0)
- **验证**: 修复后 60 次全套件 0 失败; unit_root 邻族 88/88; 三平台 fresh 重验全绿 — 主控站 MSVC **2256/2256** (656.49s) / A 站 GCC **2238/2238** (363.70s) / B 站 GCC **2238/2238** (358.67s), bundle 中继 (069264c)
- **教训两条**: ① "单独重跑通过" ≠ 无 bug — 概率性失败必须循环复现定位 (本次 8 次抓 3 败); ② **越界垃圾可以"改善"测试结果** (虚增统计量过临界值) — 方向断言通过不能反推实现正确, 数值基准锚 (对照库逐位) 才是硬锚, NP 的 NP6 对照边界 (无开源库) 使这类错位更隐蔽
...
- **修复效果**: ① A 矩阵列循环改正; ② 交叉验证 7 组全过 (v 逐位 1e-12 / u ≤1e-5 / 似然分量 ≤1e-3); ③ **θ̂/σ̂² 恢复与 R/statsmodels 同落点** — test #12/#16 断言从 "θ 仅方向" 收紧至 θ̂@5e-3+σ̂²@5e-3 全过; ④ 性能 T=1000 CSS-ML: **158.9s → 0.042s (3783×, 预算 238× 余量)**; ⑤ 新增黄金锚 #16a (B&D MA(1) θ=−0.9 四位逐字复现, v1.7 文档有锚无测试的缺口一并补上); ⑥ 16 个 timeseries 套件 **257 用例全绿零破坏** (arima 24→27 / garch_m 16→17 / zivot 15→16)
...
### v1.7.1 补丁发布 (2026-08-20)

- annotated tag `v1.7.1` 打于 `9771dd6` (v1.7.0 `0564ff6` 后 8 提交), 已推送 origin; tag 消息含补丁说明 (正确性×2 / 性能 3783× / CI 加固 / 验收口径)
- **补丁动机**: v1.7.0 tag 先于 C-3/C-4 闭环 — NP σ̂² 分母 bug 与 arma_acovf A 矩阵 bug (p≥2 中毒, θ̂/σ̂² 全错被谱等价误诊掩盖) 均落在 tag 之后; v1.7.1 将两项正确性修复 + 带状快速算法 + 三性能用例 + CI 加固一并钉进版本史
- **范围** (v1.7.0..v1.7.1, 8 提交): `5b55e8a` C-3 σ̂² 分母修复 + Stata 18 全量对照 / `a8ee98f` C-4 三性能用例 + innovations 带状快速算法 + arma_acovf 修复 / `dc3e4f2` CI gtest 发现超时加固 / `db29d6e`+`8f2c0ce`+`28e3028`+`be21c75`+`9771dd6` 文档提交
- **验收口径**: checklist 328/331 (99.1%); 三平台 2447/2447 (主控 MSVC + A/B 站 GCC); CI run #72/#73 4/4 全绿
- 同日落档: Phase 8 调研报告 v1.1 (本地 docs/research, .gitignore 约定不入库, 无 git 痕迹特此记录) — 新增 §5.1 D-1 裁决支撑 ((a) 双免费午餐 vs (b) 全押单线: 验证成本/差异化收益/时间线匹配三维度 + (b) 内部异质性: 长记忆=半个免费午餐/半参=真全押/DCC=性价比最差 + 判据收敛: 库完备度 vs 主线工具缺口); D-3 (启动节奏) 事实消解; 仅余 D-1/D-2 待裁决后进 PHASE8_SPEC
