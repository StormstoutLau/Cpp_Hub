...
| ~~C-4~~ | ✅ **已闭环 (2026-08-20)**: D-2 裁决补设三用例 → ZA 0.035s (143×)/GM 0.72s (14×)/ARIMA 0.042s (250×) 全绿; ARIMA 探针暴露 O(T³) 退化 (158.9s 击穿预算 16×) → D-4 裁决移植 statsmodels 带状快速算法 (O(T·(p+q)²)) → 交叉验证连带发现并修复 arma_acovf A 矩阵 p≥2 错位 bug (v1.7 隐藏正确性缺陷: ARMA(p≥2,q) 拟合 θ̂/σ̂² 全错, 被 "谱等价" 误诊掩盖; 修复后 #12/#16 断言收紧至 θ̂/σ̂² 数值层全过) — 全发现链见研究报告 §2.6 | §15.2-15.4 | 完成 |
