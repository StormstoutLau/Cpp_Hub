...
| 后置版本 | v1.7 多变量时序与混频 (**2458/2458** 主控站; A/B 站收尾轮待执行, 预期 2440×2) |
| Phase 7C 新增测试 | **251 个** (M0 回填 49 + M1∥M4 71 + M2 51 + M3 58 + 收尾轮 Granger 16 + 集成 6) |
| 累计测试 | 2458 (主控站 MSVC) / 预期 2440 (A/B 站 GCC, 收尾轮待验证) |
...
### 2.1 三平台矩阵 (2026-08-19 实测; A/B 站收尾轮待执行)

| 平台 | 编译器 | 测试总数 | 通过数 | 失败数 | 测试耗时 | 验证方式 | 状态 |
|------|--------|---------|--------|--------|---------|---------|------|
| 主控站 (Win10) | MSVC 19.50 (VS2026) | 2458 | 2458 | 0 | 141.63 sec (-j8) | 增量构建 + 全量 ctest | ✅ |
| A 工作站 (`scott-lau-NEX.local`) | GCC 13.3 | 2418→2440 | 2418/2418 (M3 轮 c008f46) | 0 | 425.03 sec | bundle 增量 ff + rebuild + ctest | ⏳ 收尾轮待执行 |
| B 工作站 (`scott-lau-GTR-Pro.local`) | GCC 13.3 | 2418→2440 | 2418/2418 (M3 轮 c008f46) | 0 | 434.09 sec | 同法 (mDNS 通道) | ⏳ 收尾轮待执行 |

> M0-M3 四轮 (229 用例) 三平台已全绿闭环; 收尾轮 22 用例 (Granger 16 + 集成 6) 主控站已绿, A/B 站因收尾期命令执行通道间歇性失效顺延 (条件 C-1, checklist §17.2)。

### 2.2 GitHub Actions CI (run #60/#61, 67b5450/d581f30)
...
> run #60 首次承载 M1∥M4 全量 2327 用例双平台绿; 收尾轮 commit 推送后需追加 CI run 确认 (条件 C-2)。
...
| 收尾 | Granger (16) + 端到端集成 (6) | 22 | **2458** | 2440 (待验证) | 收尾轮 |
...
### 9.1 v1.7 发布前条件 (checklist §17.2 C-1~C-4)

| # | 事项 | 优先级 |
|---|------|--------|
| C-1 | A/B 站收尾轮验证 (预期 2440/2440×2) | 高 |
| C-2 | 收尾轮 commit 推送 + CI run 确认 | 高 |
| C-3 | Stata 装机后补 NP MAIC 1e-10 硬断言 (管线就绪) | 中 (v1.8 前) |
| C-4 | 性能独立用例补设 (ZA/GM/ARIMA 规模用例) 或 N/A-级放行留档 | 低 (v1.8 评估) |
...
| 维度 | 权重 | 状态 | 备注 |
|------|------|------|------|
| 测试通过率 | 25% | ✅ | 主控 2458/2458 零退化; A/B 站 M0-M3 全绿, 收尾轮 C-1 |
...
| CI/跨平台 | 5% | ⏳ | run #60/#61 绿; 收尾轮 C-2 |
| **总计** | **100%** | **✅ 有条件通过** | 331 项 324 过 (97.9%), 未过 7 项全为预批占位/N-A 级放行 |

**验收结论**: ✅ **有条件通过** — Phase 7C v1.7 在功能、数值、幻觉点、ADR、scope、规范、复用、风险八个核心维度 100% 达标; 主控站全量 2458/2458 零退化。待条件 C-1/C-2 (A/B 站收尾轮 + CI 确认, 命令通道恢复后执行) 闭环即可进入 v1.7 发布; C-3/C-4 为 v1.8 前非阻塞项。
