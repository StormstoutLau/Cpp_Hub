...
// 2D uniform grid (x = log-S, v = variance)
// Uniform grid chosen for ADI because tridiagonal structure is preserved.
class FDMGrid2D {
public:
    ...
    Real x(Size i) const noexcept { return x_min_ + static_cast<Real>(i) * dx_; }
    Real v(Size j) const noexcept { return v_min_ + static_cast<Real>(j) * dv_; }
...
// Each sub-operator is discretized via central differences on a uniform grid.
...
inline HestonOperatorCoeffs compute_heston_coeffs(const FDMGrid2D& grid,
    ...
    Real dx = grid.dx(), dv = grid.dv();
    Real inv_2dx = 1.0 / (2.0 * dx);
    Real inv_2dv = 1.0 / (2.0 * dv);
    Real inv_dx2 = 1.0 / (dx * dx);
    Real inv_dv2 = 1.0 / (dv * dv);
    ...
    for (Size j = 0; j < nv; ++j) {
        Real v_j = grid.v(j);
        Real half_v = 0.5 * v_j;
        Real drift_x = p.r - p.q - half_v;
        Real drift_v = p.kappa * (p.theta - v_j);
        Real half_xi2_v = 0.5 * p.xi * p.xi * v_j;
        for (Size i = 0; i < nx; ++i) {
            if (i > 0 && i < nx - 1) {
                c.a_x[j][i] = half_v * inv_dx2 - drift_x * inv_2dx;
                c.b_x[j][i] = -v_j * inv_dx2;
                c.c_x[j][i] = half_v * inv_dx2 + drift_x * inv_2dx;
            }
            if (j > 0 && j < nv - 1) {
                c.a_v[j][i] = half_xi2_v * inv_dv2 - drift_v * inv_2dv;
                c.b_v[j][i] = -p.xi * p.xi * v_j * inv_dv2;
                c.c_v[j][i] = half_xi2_v * inv_dv2 + drift_v * inv_2dv;
            }
...
// Apply only L_x to V (interior points; boundary remains 0)
...
// Apply only L_v to V (interior points; boundary remains 0)
